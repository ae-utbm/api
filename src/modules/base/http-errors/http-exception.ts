...
export abstract class I18nHttpException extends Error implements OutputErrorResponseDto {
	 ...
	 */
	constructor(
		keys: ({ key: PathImpl2<I18nTranslations>; args?: TranslateOptions['args'] } | PathImpl2<I18nTranslations>)[],
	);
	...
	constructor(...val: unknown[]) {
		...
		if (val.length === 1 && Array.isArray(val[0])) {
			...
			if (val[0].length > 0) {
				const keys = val[0] as (
					| { key: PathImpl2<I18nTranslations>; args?: TranslateOptions['args'] }
			...
			const key = val[0] as PathImpl2<I18nTranslations>;
